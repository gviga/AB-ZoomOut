...
import numpy as np
from dataload_delta import Surr12kModelNetDataLoader
from tqdm import tqdm
...
TRAIN_DATASET = Surr12kModelNetDataLoader(root=DATA_PATH, npoint=1000, split='train',
                                                    normal_channel=False, augm = True)
TEST_DATASET = Surr12kModelNetDataLoader(root=DATA_PATH, npoint=1000, split='test',
                                                    normal_channel=False, augm = True)
...
basisNet = DeltaConvBasis(in_channels=3, k=40, conv_channels=[64, 128, 256], mlp_depth=2, embedding_size=512, num_neighbors=20, grad_regularizer=0.001, grad_kernel_width=0.5)
# Optimizer
