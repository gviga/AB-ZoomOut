...
checkpoint = torch.load(outf + '/basis_model_best_20.pth')
...
for epoch in range(400):
    ...
    if eval_loss <  best_eval_loss:
        ...
        torch.save(classifier.state_dict(), '%s/desc_model_best_20_test.pth' % (outf))
        torch.save(basis.state_dict(), '%s/basis_model_best_20_test.pth' % (outf))

    train_losses.append(train_loss)
    eval_losses.append(eval_loss)

    np.save(outf+'/train_losses_desc_20_test.npy',train_losses)
    np.save(outf+'/eval_losses_desc_20_test.npy',eval_losses)
