...
class PointNetfeat(nn.Module):
    ...
    def __init__(self, global_feat = True, feature_transform = False):
        ...
        #self.bn sono i livelli di BatchNorm
...
############ BASIS REFINEMENT NETWORK

#costruisco le architetture
class PointNetBasisRefinementfeat(nn.Module):
    #costruzione dell'architettura
    def __init__(self, global_feat = True, feature_transform = False):
        super(PointNetBasisRefinementfeat, self).__init__()
        #self.conv rappresentano i livelli di convoluzione
        #torch.nn.Conv1d(in_channels, out_channels, kernel_size)
        self.conv1 = torch.nn.Conv1d(3, 64, 1)
        self.conv2 = torch.nn.Conv1d(64, 64, 1)
...
#architettura per le basi
class PointNetBasisRefinement(nn.Module):
    def __init__(self, k = 10, feature_transform=False):
        super(PointNetBasisRefinement, self).__init__()
        self.k = k
        self.feature_transform=feature_transform
        self.feat = PointNetfeat(global_feat=False, feature_transform=feature_transform)
        self.conv1 = torch.nn.Conv1d(1280, 512, 1)
...
############ DESCRIPTORS REFINEMENT NETWORK
class PointNetfeatDescRefinement(nn.Module):
    def __init__(self, global_feat = True, feature_transform = False):
        super(PointNetfeatDescRefinement, self).__init__()
        self.conv1 = torch.nn.Conv1d(3, 64, 1)
        self.conv2 = torch.nn.Conv1d(64, 64, 1)
        self.conv3 = torch.nn.Conv1d(64, 64, 1)
        self.conv4 = torch.nn.Conv1d(64, 128, 1)
...
class PointNetDescRefinement(nn.Module):
    def __init__(self, k = 40, feature_transform=False):
        super(PointNetDescRefinement, self).__init__()
        self.k = k
        self.feature_transform=feature_transform
        self.feat = PointNetfeatDesc(global_feat=False, feature_transform=feature_transform)
        self.conv1 = torch.nn.Conv1d(1280, 512, 1)
